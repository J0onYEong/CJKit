...
struct DynamicText: View {
    ...
    var body: some View {
        ...
        GeometryReader { geo in
            ...
            UILabelRepresentable(content, weight: weight, viewSize: geo.size, lineCount: lineCount, kern: kern, linespacing: linespacing, textAlignment: textAlignment)
        }
...
private extension DynamicText {
    ...
    struct UILabelRepresentable: UIViewRepresentable {
        ...
        init(_ content: String, weight: UIFont.Weight, viewSize: CGSize, lineCount: Int, kern: CGFloat = 0.0, linespacing: CGFloat = 5.0, textAlignment: NSTextAlignment = .left) {
            self.content = content
        ...
        func updateUIView(_ uiView: UITextView, context: Context) {
            ...
            while(true) {
                ...
                if textSize.width > viewSize.width*CGFloat(lineCount)-errorInset.width || textSize.height > viewSize.height/CGFloat(lineCount) - linespacing*CGFloat(lineCount-1) {
                    ...
                    attributedString.addAttributes(coordinator.stringAttributes, range: NSRange(location: 0, length: attributedString.length))
